const Slide = ({ bg, image, title, description }) => {
	return (
			...
			<div
				className={'w-screen sm:w-screen flex'}
				style={{ backgroundColor: `#${bg}` }}
			>
				<div className="flex-1 sm:h-96">
					<img src={image} alt={title} />
				</div>
				<div className="flex-1 mt-16 sm:flex flex-col items-center justify-center text-center">
					<h1 className="text-xl font-semibold sm:text-7xl">{title}</h1>
					<p className="text-lg my-2 tracking-normal leading-tight sm:text-xl font-medium my-14 tracking-wide">
						{description}
					</p>
